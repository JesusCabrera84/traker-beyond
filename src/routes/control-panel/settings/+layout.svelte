<script>
	import ProductSectionLayout from '$lib/components/control-panel/ProductSectionLayout.svelte';

	const tabs = [
		{ href: '/control-panel/settings/webhooks', label: 'Webhooks', icon: 'link' },
		{ href: '/control-panel/settings/integrations', label: 'Integraciones', icon: 'puzzle' },
		{ href: '/control-panel/settings/audit', label: 'Auditoría', icon: 'shield' }
	];
</script>
...
<ProductSectionLayout
	...
	sectionSubtitle="Webhooks, integraciones de terceros y pista de auditoría"
	{tabs}
>
	<svelte:fragment slot="icon">
		<svg class="h-5 w-5 stroke-indigo-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5">
			<path
