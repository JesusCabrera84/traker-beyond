# Release Guide — geminis-labs-web-page

## Version source of truth

- **Package version:** `package.json` → `version`
- **Git tags:** annotated tags `v*.*.*` (e.g. `v1.3.5`)
...
- GitHub Actions secrets configured (see [.github/DEPLOYMENT.md](../.github/DEPLOYMENT.md))
...
| ------------ | ------------------------------- | --------------------------------------------------------------- |
| `ci.yml`     | PR + push to `develop`/`master` | Lint, type-check, tests, coverage, build, audit, security scans |
...
2. Re-deploy by pushing the previous tag again, or run the deploy workflow manually with `workflow_dispatch` if enabled.
